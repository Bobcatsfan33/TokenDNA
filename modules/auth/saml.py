...
* ``SAML_IDP_ENTITY_ID``  — exact trusted assertion issuer.
...
def consume_authn_request(request_id: str, relay_state: str, cfg: SAMLConfig | None = None) -> None:
    ...
    with _cursor() as conn:
        ...
        # The conditional update is the replay boundary. A preceding SELECT
        # alone is racy: two ACS workers can both observe an unconsumed row.
        cur.execute(
            """
            UPDATE saml_request_state
            SET consumed_at=?
...
def _is_unique_violation(exc: Exception) -> bool:
    """Recognize SQLite and Postgres unique violations without coupling backends."""
...
def record_assertion_replay(assertion_id: str, issuer: str, name_id: str, not_after: str | None, cfg: SAMLConfig | None = None) -> None:
    ...
    try:
        # The primary key is the concurrency control. Avoid a SELECT/INSERT
        # check-then-act race and translate the losing insert into a safe error.
        with _cursor() as conn:
            AdaptedCursor(conn.cursor()).execute(
                """
        ...
        if _is_unique_violation(exc):
            raise SAMLError("SAML assertion replay detected.") from exc
        raise
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata"
                     entityID={quoteattr(cfg.sp_entity_id)}>
  <md:SPSSODescriptor AuthnRequestsSigned="false"
                      WantAssertionsSigned="true"
                      protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol">
    <md:NameIDFormat>{escape(cfg.name_id_format)}</md:NameIDFormat>
    <md:AssertionConsumerService
        Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST"
        Location={quoteattr(cfg.sp_acs_url)}
        index="0"
        isDefault="true"/>
  </md:SPSSODescriptor>
</md:EntityDescriptor>
...
def build_authn_request(
    ...
    *,
    ...
    xml = (
        ...
        f'AssertionConsumerServiceURL={quoteattr(cfg.sp_acs_url)} '
        f'ProtocolBinding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST">'
        f'<saml:Issuer>{escape(cfg.sp_entity_id)}</saml:Issuer>'
        f'<samlp:NameIDPolicy Format={quoteattr(cfg.name_id_format)} AllowCreate="true"/>'
...
def parse_assertion(
    ...
    *,
    ...
    if not cfg.idp_entity_id:
        raise SAMLError("SAML_IDP_ENTITY_ID is not configured — cannot pin the assertion issuer.")
    ...
    if not issuer_data or any(issuer != cfg.idp_entity_id for issuer in issuer_data):
        raise SAMLError("SAML assertion issuer does not match SAML_IDP_ENTITY_ID.")
