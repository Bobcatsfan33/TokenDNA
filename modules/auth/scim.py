...
def init_db() -> None:
    ...
    with _cursor() as conn:
        ...
        cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_scim_users_tenant_name ON scim_users(tenant_id, lower(user_name))")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_scim_groups_tenant ON scim_groups(tenant_id)")
        cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_scim_groups_tenant_name ON scim_groups(tenant_id, lower(display_name))")
...
def _audit(action: str, *, tenant_id: str, subject: str, resource: str, detail: dict[str, Any] | None = None) -> None:
    try:
        ...
        # Provisioning succeeded, so this must be operationally visible. The
        # audit backend is not transactionally coupled to the SCIM datastore.
        logger.error("SCIM audit emission failed", exc_info=True)
...
def _raise_uniqueness(exc: Exception, resource: str) -> None:
    if _is_unique_violation(exc):
        raise SCIMError(409, f"{resource} already exists", scimType="uniqueness") from exc
    raise exc


def _assert_version(existing: dict[str, Any], if_match: str | None) -> None:
    """Apply RFC 7644 conditional-write semantics when If-Match is supplied."""
    if if_match and if_match.strip() not in {"*", str(existing["meta"]["version"])}:
        raise SCIMError(412, "Resource version does not match If-Match")


def _patch_touches_attribute(patch_doc: dict[str, Any], attribute: str) -> bool:
    ...
    for operation in patch_doc.get("Operations") or []:
        ...
        if path:
            root = path.split(".", 1)[0].split("[", 1)[0].casefold()
            if root == wanted:
                return True
        elif isinstance(operation.get("value"), dict):
            if any(str(key).casefold() == wanted for key in operation["value"]):
                return True
    return False


def _validated_members(raw: Any, tenant_id: str) -> list[Any]:
    members = list(raw or [])
    ids = {member_id for member in members if (member_id := _member_id(member))}
    if len(ids) != len(members):
        raise SCIMError(400, "Every group member must contain a user value", scimType="invalidValue")
    if not ids:
        return members
    placeholders = ",".join("?" for _ in ids)
    with _cursor() as conn:
        rows = AdaptedCursor(conn.cursor()).execute(
            f"SELECT id FROM scim_users WHERE tenant_id=? AND id IN ({placeholders})",
            (tenant_id, *sorted(ids)),
        ).fetchall()
    found = {row["id"] for row in rows}
    if found != ids:
        raise SCIMError(400, "Group members must reference users in the same tenant", scimType="invalidValue")
    return members
...
def replace_user(
    ...
    *,
    ...
    _assert_version(existing, if_match)
...
def patch_user(
    ...
    *,
    ...
    existing = get_user(user_id, tenant_id=tenant_id)
    _assert_version(existing, if_match)
    # Patch manual roles, not the union of manual and group-derived roles.
    # Otherwise an unrelated patch can persist a temporary group privilege.
    patch_base = {**existing, "roles": _fetch_manual_roles(user_id, tenant_id)}
    try:
        patched = apply_patch(patch_base, patch_doc)
    ...
    if not _patch_touches_attribute(patch_doc, "roles"):
        merged.pop("roles", None)
    return replace_user(user_id, merged, tenant_id=tenant_id, if_match=if_match)


def delete_user(user_id: str, *, tenant_id: str, if_match: str | None = None) -> None:
    existing = get_user(user_id, tenant_id=tenant_id)
    _assert_version(existing, if_match)
    with _cursor() as conn:
        cur = AdaptedCursor(conn.cursor())
...
def create_group(payload: dict[str, Any], *, tenant_id: str) -> dict[str, Any]:
    ...
    version = _new_version()
    members = _validated_members(payload.get("members"), tenant_id)
    try:
        with _cursor() as conn:
            AdaptedCursor(conn.cursor()).execute(
            """
...
def patch_group(
    ...
    *,
    ...
    members = _validated_members(merged.get("members"), tenant_id)
    now = _now_iso()
    try:
        with _cursor() as conn:
            AdaptedCursor(conn.cursor()).execute(
...
def delete_group(group_id: str, *, tenant_id: str, if_match: str | None = None) -> None:
    existing = get_group(group_id, tenant_id=tenant_id)
    _assert_version(existing, if_match)
    with _cursor() as conn:
        AdaptedCursor(conn.cursor()).execute(
