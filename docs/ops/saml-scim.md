...
SAML_IDP_ENTITY_ID=https://customer-idp.example/entity-id
...
TokenDNA requires signed assertions and refuses SAML assertion parsing when `python3-saml` is unavailable. The production preflight fails if SAML is configured without the pinned IdP entity ID, IdP signing certificate, IdP SSO URL, HTTPS SP URLs, RelayState host allowlist, or runtime SAML dependency.
...
7. Copy the IdP entity ID, SSO URL, and active signing certificate into TokenDNA secrets.
...
### Role mapping and tenant isolation
...
When a mapped group is created, patched, or deleted, each affected user's
effective roles are recomputed from current membership. Manually assigned
SCIM `roles` are preserved separately and unioned with group-derived roles.
The default is least privilege: unmapped users gain no elevated role.
