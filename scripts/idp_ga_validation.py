...
def run_validation(base_url: str, *, provider: str, api_key: str | None, bearer: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        user_etag = str(create.headers.get("etag") or "")
        _record(
            steps,
            "scim_user_create",
            create.status_code == 201 and bool(user_id) and user_etag == body.get("meta", {}).get("version"),
            f"status={create.status_code} etag={bool(user_etag)}",
        )
    except Exception as exc:  # noqa: BLE001
    ...
    if user_id:
        try:
            patch = _request(
                ...
                headers={**scim_headers, "If-Match": user_etag},
                json_body={
            ...
            body = patch.json()
            user_etag = str(patch.headers.get("etag") or user_etag)
    ...
    try:
        ...
        group_etag = str(group.headers.get("etag") or "")
        _record(
            steps,
            "scim_group_create",
            group.status_code == 201 and bool(group_id) and group_etag == body.get("meta", {}).get("version"),
            f"status={group.status_code} etag={bool(group_etag)}",
        )
    except Exception as exc:  # noqa: BLE001
    ...
    if user_id and group_id:
        try:
            patch_group = _request(
                ...
                headers={**scim_headers, "If-Match": group_etag},
                json_body={
                    "schemas": ["urn:ietf:params:scim:api:messages:2.0:PatchOp"],
            ...
            group_etag = str(patch_group.headers.get("etag") or group_etag)
